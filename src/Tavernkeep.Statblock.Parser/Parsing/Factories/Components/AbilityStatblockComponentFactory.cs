...
{
	internal sealed class AbilityStatblockComponentFactory : IStatblockComponentFactory
	{
		public IStatblockComponent GetComponent(List<IToken> source, ref int i)
		{
			var tokens = ParsingUtilities.Collect(source, ref i, source.Count, s => StatblockRegexes.AbilitySubKeywords().IsMatch(s));
			return new AbilityStatblockComponent(tokens);
		}
	}
}
