...
{
	internal sealed class AttackStatblockComponentFactory : IStatblockComponentFactory
	{
		public IStatblockComponent GetComponent(List<IToken> source, ref int i)
		{
			var tokens = ParsingUtilities.Collect(source, ref i, source.Count, s => StatblockRegexes.AttackSubKeywords().IsMatch(s));
			return new AttackStatblockComponent(tokens);
		}
	}
}
