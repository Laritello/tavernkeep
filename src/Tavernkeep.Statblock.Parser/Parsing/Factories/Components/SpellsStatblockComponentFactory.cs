...
{
	internal sealed class SpellsStatblockComponentFactory : IStatblockComponentFactory
	{
		public IStatblockComponent GetComponent(List<IToken> source, ref int i)
		{
			var tokens = ParsingUtilities.Collect(source, ref i, source.Count, s => StatblockRegexes.SpellsLevel().IsMatch(s) || StatblockRegexes.SpellsKeywords().IsMatch(s));
			return new SpellsStatblockComponent(tokens);
		}
	}
}
