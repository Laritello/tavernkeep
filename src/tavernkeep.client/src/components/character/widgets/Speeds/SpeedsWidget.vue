...
import SpeedsEditDialog from '@/components/dialogs/edit/SpeedsEditDialog.vue';
...
import SpeedsWidgetItem from './SpeedsWidgetItem.vue';

const { t } = useI18n();

const emits = defineEmits<{
    changed: [value: Record<SpeedType, SpeedEditDto>];
}>();

const { speeds } = defineProps<{
    speeds: Record<SpeedType, Speed>;
}>();

async function showEditSpeedsDialog() {
    const modal = useModal();
    const result = await modal.show(SpeedsEditDialog, {
        speeds,
    });
...
<template>
    <div class="bg-base-100 border-2 border-base-300 rounded-xl border-inherit flex flex-col p-2 w-full lg:max-w-md">
        <div class="flex flex-row justify-center items-center">
            <h2 class="mr-1 text-lg font-semibold select-none">{{ t('sections.speeds') }}</h2>
            <button class="btn btn-sm btn-circle btn-ghost" @click="showEditSpeedsDialog()">
