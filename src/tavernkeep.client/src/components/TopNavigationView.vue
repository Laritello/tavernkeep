...
async function performPerceptionCheck() {
    if (user.activeCharacter.value !== undefined) {
        const message = await api.performSkillCheck(user.activeCharacter.value.id, 'Perception', RollType.Public);
...
<template>
    <div class="navbar bg-base-100 z-10 min-h-fit border-b-2 border-base-300 p-0">
        <div v-if="user.activeCharacter.value !== undefined" class="flex flex-col w-full items-stretch">
            <div class="flex flex-row pt-1 px-1">
                ...
                <div class="flex-none w-24">
                    <div class="flex flex-col gap-1">
                        <div class="flex flex-row">
                            <PerceptionBadge
                                :perception="user.activeCharacter.value.perception"
                                class="w-12"
                                @roll="performPerceptionCheck"
                            />
                            <ArmorClassBadge :armor="user.activeCharacter.value.armor" class="w-12" />
                        </div>
                        <HeroPoints
                            class="flex-none"
                            :amount="user.activeCharacter.value.heroPoints"
            ...
            <div
                class="transition-all overflow-hidden ease-in-out duration-300 px-1"
                ...
                <div class="flex flex-row">
                    <div class="flex flex-col border border-t-0 rounded-b-lg conditions-list w-24 mb-1">
                        <div v-if="user.activeCharacter.value.conditions.length > 0" class="px-1 overflow-auto">
                            <p
                                v-for="item in user.activeCharacter.value.conditions"
                                :key="item.name"
