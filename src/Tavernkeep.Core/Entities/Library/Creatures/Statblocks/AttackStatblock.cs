...
{
	public sealed class AttackStatblock(List<StatblockSection> sections) : IStatblock
	{
		private string html_template = @"<p class='hang'>{{ for section in sections }}<strong>{{ section.header }}</strong>{{ for span in section.spans }} {{ span.html }} {{ end }} {{ end }}</p>";
