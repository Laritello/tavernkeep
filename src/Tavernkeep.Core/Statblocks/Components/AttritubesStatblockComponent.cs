...
{
	public sealed class AttributesStatblockComponent(List<IToken> tokens) : IStatblockComponent
	{
		...
		{
			return new AttributesStatblockComponent(Tokens.Select(x => x is IModifiableOriginToken origin ? origin.ToModifable() : x.Copy()).ToList())
			{
