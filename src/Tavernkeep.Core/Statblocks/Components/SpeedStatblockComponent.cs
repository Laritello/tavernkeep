...
{
	public sealed class SpeedStatblockComponent(List<IToken> tokens) : IStatblockComponent
	{
		...
		{
			return new SpeedStatblockComponent(Tokens.Select(x => x is IModifiableOriginToken origin ? origin.ToModifable() : x.Copy()).ToList())
			{
