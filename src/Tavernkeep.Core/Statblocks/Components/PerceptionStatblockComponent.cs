...
{
	public sealed class PerceptionStatblockComponent(List<IToken> tokens) : IStatblockComponent
	{
		...
		{
			return new PerceptionStatblockComponent(Tokens.Select(x => x is IModifiableOriginToken origin ? origin.ToModifable() : x.Copy()).ToList())
			{
