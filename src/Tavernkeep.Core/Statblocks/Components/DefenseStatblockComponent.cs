...
{
	public sealed class DefenseStatblockComponent(List<IToken> tokens) : IStatblockComponent
	{
		...
		{
			return new DefenseStatblockComponent(Tokens.Select(x => x is IModifiableOriginToken origin ? origin.ToModifable() : x.Copy()).ToList())
			{
