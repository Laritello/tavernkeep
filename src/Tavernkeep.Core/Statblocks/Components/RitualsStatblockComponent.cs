...
{
	public sealed class RitualsStatblockComponent(List<IToken> tokens) : IStatblockComponent
	{
		...
		{
			return new RitualsStatblockComponent(Tokens.Select(x => x is IModifiableOriginToken origin ? origin.ToModifable() : x.Copy()).ToList())
			{
