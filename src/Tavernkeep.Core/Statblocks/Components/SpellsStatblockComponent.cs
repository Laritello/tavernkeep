...
{
	public sealed class SpellsStatblockComponent(List<IToken> tokens) : IStatblockComponent
	{
		...
		{
			return new SpellsStatblockComponent(Tokens.Select(x => x is IModifiableOriginToken origin ? origin.ToModifable() : x.Copy()).ToList())
			{
